	...
	const services = [
		{
			...
			subtitle: "Contact sales to purchase a custom plan.",
		},
		{
			icon: "/icons/support.svg",
			title: "Support & training",
			subtitle: "Connect with support and training resources",
		},
		{
			icon: "/icons/press-enquiry.svg",
			title: "Press inquiries",
			subtitle: "For all press, analyst, and speaking opportunities",
		},
	]
	return (
		<MainLayout>
			...
			<div className="container flex flex-col md:flex-row justify-between space-y-8 md:space-y-0 md:space-x-8 px-4 md:px-0">
				{services?.map((service, i) => (
					<div
						key={i}
						className="shadow-custom5 p-6 border border-[#eaeaea] flex-grow"
					>
						...
						<div className="border-t-2 border-primary pt-4 mt-4 pb-16">
							<h1 className="text-[30px]">{service.title}</h1>
							<p className="text-[16px] mt-4">{service.subtitle}</p>
						</div>
					</div>
				))}
			</div>
			...
			<div className="py-24 px-4 md:px-0">
				...
				<p className="text-[16px] text-center mt-2">
					Fill out the form below and a representative will get back to you as
					soon as possible.
				</p>
				<form className="flex flex-col justify-center space-y-16">
					<div className="container flex flex-col md:flex-row space-y-8 md:space-y-0 items-center  mt-16 md:space-x-16 ">
						<div className="flex flex-col w-full md:w-1/2 space-y-8">
							...
							<input
								placeholder="Company"
								className="bg-secondary border border-[#eaeaea] outline-none py-[6px] px-2"
							/>
							<input
						...
						<div className="flex flex-col w-full md:w-1/2 h-full">
							<textarea
								className="bg-secondary border border-[#eaeaea] outline-none py-1 px-2 h-80 text-gray-400"
